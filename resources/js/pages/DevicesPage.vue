        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-row mb-4 justify-between">
                ...
                <div class="justify-center self-center">
                    <input type="text" name="deviceName" id="deviceName" placeholder="Поиск" class="input input-bordered w-full input-sm" autocomplete="on">
                </div>
            </div>
            ...
            <div class="flex justify-center">
                <div class="join">
                    <button class="join-item btn" @click="this.prevPage">«</button>
                    <button class="join-item btn">{{ this.page }}</button>
                    <button class="join-item btn" @click="this.nextPage">»</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
        ...
        pages() {
            return Math.ceil(this.getDevices.length / this.devicePerPage);
        },
        paginatedDevices() {
            let from = (this.page - 1) * this.devicePerPage;
            let to = from + this.devicePerPage;
            return this.getDevices.slice(from, to);
        }
    },
    methods: {
        ...mapActions([
